...
namespace Match3
{
    public class GameGrid : MonoBehaviour
    {
        ...
        private bool isCheckingDeadlock;
        ...
        private bool IsDeadlock()
        {
            for (int x = 0; x < xDim; x++)
            {
                for (int y = 0; y < yDim; y++)
                {
                    ...
                    if (piece != null && (IsSpecialType(piece.PieceType) || piece.PieceType == PieceType.Rainbow))
                        return false;
                }
            }
            ...
            for (int x = 0; x < xDim; x++)
            {
                for (int y = 0; y < yDim; y++)
                {
                    if (x < xDim - 1 && HasValidSwap(x, y, x + 1, y))
                        return false;

                    if (y < yDim - 1 && HasValidSwap(x, y, x, y + 1))
                        return false;
                }
            }
            return true;
        ...
        private bool HasValidSwap(int x1, int y1, int x2, int y2)
        {
            ...
            if (IsSpecialType(piece1.PieceType) || IsSpecialType(piece2.PieceType) || piece1.PieceType == PieceType.Rainbow || piece2.PieceType == PieceType.Rainbow)
                return true;
            ...
            bool hasMatch =
                GetMatch(piece1, x2, y2) != null ||
                GetMatch(piece2, x1, y1) != null /*||
        ...
        private IEnumerator ShufflePieces()
        {
            ...
            for (int x = 0; x < xDim; x++)
            {
                for (int y = 0; y < yDim; y++)
                {
                    ...
                    if (currentPiece == null || !currentPiece.IsMovable() || currentPiece.PieceType != PieceType.Normal ||
                        currentPiece.PieceType == PieceType.Empty)
                        continue;

                    if (index >= movablePieces.Count) break;
                    GamePiece newPiece = movablePieces[index];
                    newPiece.MovableComponent.Move(x, y, fillTime);
        ...
        protected IEnumerator SwapPieces(GamePiece piece1, GamePiece piece2)
        {
            ...
            if (!IsAdjacent(piece1, piece2)) yield break;
            if (!piece1.IsMovable() || !piece2.IsMovable()) yield break;

            Debug.Log($"尝试交换: Piece1({piece1.X}, {piece1.Y}) Movable={piece1.IsMovable()}, Piece2({piece2.X}, {piece2.Y}) Movable={piece2.IsMovable()}");
            ...
            bool hasSpecialMatch =
                piece1.PieceType == PieceType.Rainbow ||
                piece2.PieceType == PieceType.Rainbow ||
                IsSpecialType(piece1.PieceType) ||
                IsSpecialType(piece2.PieceType);
            ...
            {
                yield return new WaitForSeconds(fillTime);
                HandleSpecialPieceSwap(piece1, piece2);
                HandleSpecialPieceSwap(piece2, piece1);
            ...
            while (currentRecursion < maxRecursion)
            {
                ...
                for (int y = 0; y < yDim; y++)
                {
                    for (int x = 0; x < xDim; x++)
                    {
                        ...
                        {
                            if(IsSquarePiece(piece.X,piece.Y,piece.ColorComponent.Color))
                            {
                                isSquare = true;
                                break;
                            }
                        ...
                        if (match.Count >= 4)
                        {
                            ...
                            if (isSquare)
                            {
                                ...
                                specialPieceX = match[0].X;
                                specialPieceY = match[0].Y;
                            }
        ...
        private bool IsSquarePiece(int x, int y, ColorType color)
        {
            ...
            return CheckExtendedSquare(x, y, color);
        }

        private bool CheckExtendedSquare(int x, int y, ColorType color)
        {
                ...
                for (int startY = 0; startY < yDim - 1; startY++)
                {
                    ...
                    if (startY > 0)
                    {
                        for (int dx = 0; dx < 2; dx++)
                        {
                            ...
                            int checkY = startY - 1;
                            if (checkX < xDim && checkY >= 0 &&
                                _pieces[checkX, checkY].IsColored() &&
                                _pieces[checkX, checkY].ColorComponent.Color == color)
                                return true;
                        }
                    }
        ...
        private bool CheckSquare(int startX, int startY, ColorType color)
        {
            if (startX >= xDim - 1 || startY >= yDim - 1) return false;

            return
                _pieces[startX, startY].IsColored() && _pieces[startX, startY].ColorComponent.Color == color &&
                _pieces[startX + 1, startY].IsColored() && _pieces[startX + 1, startY].ColorComponent.Color == color &&
                _pieces[startX, startY + 1].IsColored() && _pieces[startX, startY + 1].ColorComponent.Color == color &&
                _pieces[startX + 1, startY + 1].IsColored() && _pieces[startX + 1, startY + 1].ColorComponent.Color == color;
        ...
        {
            GamePiece piece = _pieces[x, y];
            if (piece.PieceType == PieceType.Unfillable) return false;
        ...
        public List<GamePiece>GetPiecesOfObstacleType(ObstacleType type)
        {
            ...
            for (int x = 0; x < xDim; x++)
            {
                ...
                {
                    GamePiece piece = _pieces[x, y];
                    if (piece != null && piece.ObstacleType == type)
                        pieces.Add(piece);
                }
            }
            return pieces;
